...
describe('isAccountStatusAllowed', () => {
    test('should allow when user status is equal to minimum required', () => {
        expect(
            isAccountStatusAllowed(AccountStatus.ACTIVE, AccountStatus.ACTIVE),
        ).toBe(true);
    ...
    test('should return false if either status is not found in sortedRoles', () => {
        expect(
            isAccountStatusAllowed('invalid' as any, AccountStatus.ACTIVE),
        ).toBe(false);
        expect(
            isAccountStatusAllowed(AccountStatus.ACTIVE, 'invalid' as any),
        ).toBe(false);
    });
});

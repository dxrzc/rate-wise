...
export class AuthService {
    ...
    async signUp(signUpInput: SignUpInput, req: RequestContext): Promise<User> {
        this.logger.log(`New account creation attemp for ${signUpInput.email}`);
        ...
        this.logger.log(`Account ${signUpInput.email} created successfully `);
        return user;
    }

    async signIn(credentials: SignInInput, req: RequestContext): Promise<User> {
        this.logger.log(`Sign in attemp for email ${credentials.email}`);
    ...
    async signOut(req: RequestContext): Promise<void> {
        this.logger.log(`Sign out for session ${req.sessionID}`);
        await this.sessionService.deleteSession(req);
        this.logger.log(`User ${req.session.userId} signed out`);
    }
    ...
    async signOutAll(
        ...
        this.logger.log(`Attempting to close all sessions for user: ${userId}`);
        ...
        this.logger.log(`All sessions closed for userId: ${userId}`);
    }
}
